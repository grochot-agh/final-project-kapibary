    ...
    <div class="posty">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $reserved = $row["zarezerwowane"] == 0 ? "Nie" : substr($row["mail_z"], 0, 3) . str_repeat("*", strlen($row["mail_z"]) - 3);
                $zdj = $row["zdjecie"];
                ...
                echo "<p class='reserved'>Zarezerwowane: " . $reserved . "</p>"; // Tutaj wstawiono zmienioną część kodu
                echo "</div>";
                echo "</div>";
                echo "</div>";
            }
